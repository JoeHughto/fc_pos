...
if (($_POST['submit'] == 'submit') || ($_POST['submit'] == 'close') || ($_POST['close'] == 1)) {
    ...
    for ($icount = 1000000; isset($description[$icount]); $icount++) {
        ...
        if ($bad['description'] = ereg("[0-9]{12}", $description[$icount])) {
            ...
            $description[$icount] = mysqli_real_escape_string($cxn, $description[$icount]);
        }
        if ($bad['manufacturer'] = !checkName($manufacturer[$icount])) {
            $manufacturer[$icount] = '';
        } else {
            $manufacturer[$icount] = mysqli_real_escape_string($cxn, $manufacturer[$icount]);
        }
        if ($bad['department'] = !checkName($department[$icount])) {
            $department[$icount] = '';
        } else {
            $department[$icount] = mysqli_real_escape_string($cxn, $department[$icount]);
        }
        $UPC[$icount] = extractNums($UPC[$icount]);
        if ($bad['alternate1'] = !checkName($alternate1[$icount])) {
            $alternate1[$icount] = "ERROR%";
        } else {
            $alternate1[$icount] = mysqli_real_escape_string($cxn, $alternate1[$icount]);
        }
        if ($bad['alternate2'] = !checkName($alternate2[$icount])) {
            $alternate2[$count] = "ERROR%";
        } else {
            $alternate2[$icount] = mysqli_real_escape_string($cxn, $alternate2[$icount]);
        }
        ...
        if ($bad['description'] != true) {
            ...
            if (!$result = mysqli_query($cxn, $sql)) {
                displayErrorDie("Error #5: Unable to check description for dups<br>Query: $sql<br>Error: " . mysqli_error($cxn));
            }
        ...
        foreach ($newqty as $thisID => $q) {
            ...
            if ($q > 0) {
                ...
                if (mysqli_query($cxn, $sql)) {
                    ...
                    if (mysqli_affected_rows($cxn) == 0) { // if we have to make a new itemChange
                        $sql = "INSERT INTO itemChange
                            (itemID, invEventID, qty, cost, price)
                            VALUES
                            ('$thisID', '$IEID', '$q', '$c', '$p')";
