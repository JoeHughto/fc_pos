...
// 1.3 - adds count in and out options
  ...
  // this must be done before the header is loaded.
   ...
   // display last 3 credit transactions
   // COMMENTED OUT June 2014 to test a sleeker master page
   //displayCreditTransactions($SID, 3);
   
    // Trash button
    echo "You can get Hedons for small tasks around the store. If you do the task, push the button. Do not abuse our honor system!<br>
         <form action='index.php' method='post'>
      ...
      {//If the store is currently open vVv
		 ...
		 if($sched == 0 || ($intRow[1] != 1 && ($recRow[0] == $SID || $intRow[0] == $SID || lateReg())))
         {
			...
			else if (lateReg())
			{
				...
				echo printMemberString($curMem, 1) . " is late for this shift.</b></td></tr></table><hr>\n";
			}
         }
         else if($intRow[1] == 1 && $intRow[0] == $SID)
         {
            echo "<table border><tr><td><b>You are currently checked in for this shift</b></td></tr></table><hr>\n";
         }
         else if($intRow[1] == 1)
         {
            echo "<table border><tr><td><b> ";
            echo printMemberString($intRow[0], 1) . " is currently checked in for this shift</b></td></tr></table><hr>\n";
         }
      } // end if shiftNow > 0
   ...
   echo "<a name='account'>
         ...
         Your available Store Account Balance: " . money(getAvailBalance($_SESSION['ID'])) . "<br>
         Your current Future Packs: " . getAccountPacks($_SESSION['ID']) . "<br>
         Your current Future Pack Value: " . money(3 * getAccountPacks($_SESSION['ID'])) . "<p>
