...
// 1.1 Emails passwords when they are changes
   ...
   {
      if($_POST['sub'] > 0) // if there is data to process
      {
         // check data
         extract($_POST);
         if(strlen($newpwd1) > 0)
         ...
         if($registerUse != 1) $registerUse = 0;
         if($inventoryUse != 1) $inventoryUse = 0;
         if($memberUse != 1) $memberUse = 0;
         if($eventUse != 1) $eventUse = 0;
         if($adminUse != 1) $adminUse = 0;
         ...
         switch($contributing)
         {
            ...
            case 21 : $contribExp = "2300-01-01";
                     break;
            default : $contribExp = '0000-00-00';
         }

         // if its all good, ram it in there
         if(!in_array(TRUE, $bad))
         {
            if($sub == 1) // if there is a new member to add
            {
               // we only do the password if they are new. Don't want to reset the password for every edit
               $origpass = $password;
               $password = hash('sha256',$password);

               $sql = "INSERT INTO members
                                   (login, password, fname, lname, street, city, state, ZIP, email, phone1, phone2, DOB, registerUse, inventoryUse, memberUse, eventUse, adminUse, memberSince, workingExp, contribExp, taxexempt)
                                   VALUES
                                   ('$login', '$password', '$fname', '$lname', '$street', '$city', '$state', '$ZIP', '$email', '$phone1', '$phone2', '$DOB', '$registerUse', '$inventoryUse', '$adminUse', '$memberUse', 'eventUse', NOW(), '$workingExp', '$contribExp', '$taxexempt')";
               if(!mysqli_query($cxn, $sql))
               {
                  displayError("Error #8: Query Error<br>Query: $sql<br>SQL Error: " . mysqli_error($cxn));
               }
               else
               {
...
This email is to inform you that you have just been entered into the Worlds Apart Member Database.

You may now log into Friend Computer to administer your Credit account, sell stuff when you are on duty and all the rest of that good stuff.
...
Friend Computer";
                  ...
                  mail("$email", "Welcome New Worlds Apart Member", $message, "From: friendcomputer@worldsapartgames.com");
                  unset($login, $fname, $lname, $street, $city, $state, $ZIP, $email, $phone1, $phone2, $status, $DOB, $registerUse, $inventoryUse, $adminUse, $ID, $taxexempt);
               }
            } // end if
            else if($sub == 2) // if it is an edit
            {
               ...
               {
                  echo "Information for $fname $lname updated in database successfully!<hr>";
...
Your member information has been updated in Friend Computer, and your password has been updated
...
Friend Computer";
                  ...
                  unset($login, $fname, $lname, $street, $city, $state, $ZIP, $email, $phone1, $phone2, $status, $DOB, $registerUse, $inventoryUse, $adminUse, $memberUse, $DOBDay, $DOBMonth, $DOBYear, $taxexempt);
               }
            } // end if
         } // end if not bad
         else
   ...
   if($ID>0) // if it's an edit we set sub to 2 so that it is processed as an edit
   {
      echo "<input type='hidden' name='ID' value='$ID'>
            <input type='hidden' name='sub' value=2>\n";
   }
   else // otherwise we set sub as 1 which means that it's a new entry
   {
   ...
   echo "</td></tr>
         <tr><td>Tax exempt number (for organizations only):</td><td><input type='text' name='taxexempt' value='$taxexempt' size=15 maxlength=15></td></tr>
